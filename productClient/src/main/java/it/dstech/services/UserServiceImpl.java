...
public class UserServiceImpl implements UserService{
	...
	public User saveUser(User user) {
		try {
			URL url = new URL("http://localhost:8082/user/save");
	...
	public User getUserById(int id) {
		try {
			URL url = new URL("http://localhost:8082/user/getById/{id}");
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setDoOutput(true);
	...
	public User deleteUser(int id) {
		try {
			URL url = new URL("http://localhost:8082/user/delete/{id}");
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setDoOutput(true);
	...
	public User getUserByUsernameAndPassword(String username, String password) {
		try {
			URL url = new URL("http://localhost:8082/user/findbypasswordandusername");
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setDoOutput(true);
			conn.setRequestMethod("GET");
